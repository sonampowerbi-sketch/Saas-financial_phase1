...
try:
    ...
    st.warning("Plotly library loading. Charts will be available in a moment.")
...
if plotly_available:
    ...
    with col2:
        ...
        if 'Revenue' in daily.columns and 'Expense' in daily.columns:
            ...
            fig.update_layout(height=400, title="Daily Revenue vs Expenses")
            st.plotly_chart(fig, use_container_width=True)
    
    st.markdown("---")
else:
    st.info("📊 Charts loading... Please refresh in a moment.")

# Metrics table
st.subheader("📊 Key Metrics Summary")

metrics_df = pd.DataFrame({
    'Metric': ['Total Revenue', 'Total Expenses', 'Net Cash Flow', 'Monthly Burn', 'Runway', 
               'MRR', 'CAC', 'LTV', 'LTV/CAC Ratio', 'Total Customers', 'Avg MRR/Customer'],
    'Value': [
        f"${revenue:,.0f}",
        f"${expenses:,.0f}",
        f"${revenue - expenses:,.0f}",
        f"${monthly_burn:,.0f}",
        f"{runway:.0f} months" if runway != float('inf') else "∞",
        f"${mrr:,.0f}",
        f"${cac:,.0f}",
        f"${ltv:,.0f}",
